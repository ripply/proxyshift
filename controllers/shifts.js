...
function _getShiftsYouAreManaging_joinManagingClasses(query, on, user_id, location_id) {
    if (location_id) {
        query.innerJoin('managingclassesatlocations', function() {
            this.on('managingclassesatlocations.groupuserclass_id', '=', on);
        })
        // map sublocation_id to location_id
        .leftJoin('sublocations', function() {
            this.on('sublocations.id', '=', 'managingclassesatlocations.sublocation_id');
        })
        // confine to locations
        .innerJoin('locations', function() {
            this.on('locations.id', '=', 'managingclassesatlocations.location_id')
                .orOn('locations.id', '=', 'sublocations.location_id');
        })
        .where('locations.id', '=', location_id)
        .andWhere('managingclassesatlocations.managing', '=', controllerCommon.true)
        .innerJoin('groups', function() {
            this.on('groups.id', '=', 'locations.group_id');
        })
        .innerJoin('groupuserclasses', function() {
            this.on('groupuserclasses.id', '=', 'managingclassesatlocations.groupuserclass_id');
        })
        .innerJoin('grouppermissions', function() {
            this.on('grouppermissions.group_id', '=', 'groups.id');
        })
...
// createSelectQueryForAllColumns('Shift', 'shifts').join(', ')

const SQL_GET_MANAGING_SHIFTS_WITH_LOCATION = "\
distinct shifts.* from shifts \
 left join sublocations on sublocations.id = shifts.sublocation_id\
 inner join locations on locations.id = sublocations.location_id or\
  locations.id = shifts.location_id\
 inner join groups on groups.id = locations.group_id\
 inner join groupuserclasses on groupuserclasses.group_id = groups.id\
 left join usergroups on usergroups.group_id = groups.id\
 inner join groupuserclasstousers on\
  groupuserclasstousers.groupuserclass_id = groupuserclasses.id and\
  (groupuserclasstousers.user_id = ? or groups.user_id = ?)\
 inner join grouppermissions on grouppermissions.id = usergroups.grouppermission_id\
 left join managingclassesatlocations on\
  managingclassesatlocations.sublocation_id = sublocations.id or\
  managingclassesatlocations.location_id = locations.id\
 where\
  (\
...
function getShiftsYouAreManaging(req, res, pendingApprovalsOnly, noApplicationsOnly, approvedOnly, startShift, endShift) {
    ...
    return models.Shift.query(function(q) {
        var managingClassesAtLocationSubQuery = undefined;
        /*
        if (location_id === undefined) {
            managingClassesAtLocationSubQuery =
            ...
            managingClassesAtLocationSubQuery =
                knex.select('managingclassesatlocations.id as id')
                    .from('managingclassesatlocations')
                    // map sublocation_id to location_id
                    .leftJoin('sublocations', function() {
                        this.on('sublocations.id', '=', 'managingclasses.sublocation_id');
                    })
                    // confine to locations
                    .innerJoin('locations', function() {
                        this.on('locations.id', '=', 'managingclasses.location_id')
                            .orOn('locations.id', '=', 'sublocations.location_id');
                    })
                    .where('locations.id', '=', location_id)
                    .andWhere('managingclassesatlocations.managing', '=', controllerCommon.true)
            ...
            */
            ...
            //managingClassesAtLocationSubQuery = managingClassesAtLocationSubQuery
                //.whereIn('managingclassesatlocations.id', managingMemberOfLocationSubQuery);
        //}
        ...
        if (location_id) {
            ...
            _getShiftsYouAreManaging_joinManagingClasses(q, 'shifts.groupuserclass_id', req.user.id, req.params.location_id)
                .union(function () {
                    this.select('shifts.*')
                        .from('shifts')
                        .innerJoin('sublocations', function () {
                    ...
                    _getShiftsYouAreManaging_joinManagingClasses(this, 'shifts.groupuserclass_id', req.user.id, req.params.location_id);
                });
        } else {
            ...
            //q.raw(SQL_GET_MANAGING_SHIFTS_WITH_LOCATION);
            var user_id = req.user.id;
            //q.select(models.knex.raw(SQL_GET_MANAGING_SHIFTS_WITH_LOCATION, [user_id, user_id, user_id]));
            ...
            q.select('shifts.*')
                ...
                .joinRaw('inner join groupuserclasstousers on\
  groupuserclasstousers.groupuserclass_id = groupuserclasses.id and\
  (groupuserclasstousers.user_id = ? or groups.user_id = ?)', [user_id, user_id])
                .innerJoin('grouppermissions', function() {
            ...
            managingclassesatlocations.groupuserclass_id = shifts.groupuserclass_id\
                ...
                /*
                .where(function() {
                    this.where('managingclassesatlocations.managing', '=', models.sqlTrue)
                        .andWhere('grouppermissions.permissionlevel', '>=', managingPermissionLevel)
                        .andWhere(function() {
                            this.where('managingclassesatlocations.sublocation_id', '=', 'shifts.sublocation_id')
                                .orWhere('managingclassesatlocations.location_id', '=', 'shifts.location_id');
                        })
                        .andWhere('managingclassesatlocations.groupuserclass_id', '=', 'shifts.groupuserclass_id');
            ...
            /*
             select distinct shifts.id from shifts
             left join sublocations on sublocations.id = shifts.sublocation_id
             inner join locations on locations.id = sublocations.location_id or
             locations.id = shifts.location_id
             inner join groups on groups.id = locations.group_id
             ...
             inner join groupuserclasstousers on
             groupuserclasstousers.groupuserclass_id = groupuserclasses.id and
             (groupuserclasstousers.user_id = 5 or groups.user_id = 5)
             inner join grouppermissions on grouppermissions.id = usergroups.grouppermission_id
             left join managingclassesatlocations on
             managingclassesatlocations.sublocation_id = sublocations.id or
             managingclassesatlocations.location_id = locations.id
             where
             (
             managingclassesatlocations.managing = 1 and
             grouppermissions.permissionlevel >= 2 and
             (
             managingclassesatlocations.sublocation_id = shifts.sublocation_id or
             managingclassesatlocations.location_id = shifts.location_id
             ) and
            ...
            q.select(columns)
                ...
                .joinRaw('INNER JOIN locations ON locations.id = COALESCE(sublocations.location_id, shifts.location_id)')
                .innerJoin('groups', function() {
                    this.on('groups.id', '=', 'locations.group_id');
                })
                .leftJoin('usergroups', function() {
                ...
                .innerJoin('groupuserclasstousers', function() {
                    this.on('groupuserclasstousers.groupuserclass_id', '=', 'groupuserclasses.id')
                        .orOn(function() {
                            this.where('groupuserclasstousers.user_id', '=', req.user.id)
                                .orWhere('groups.user_id', '=', req.user.id);
                        });
                })
                .leftJoin('grouppermissions', function() {
                    this.on('grouppermissions.id', '=', 'usergroups.grouppermission_id');
                })
                .where(function() {
                    this.where('groups.user_id', '=', req.user.id)
                        .orWhere('grouppermissions.permissionlevel', '>=', managingPermissionLevel);
                })
            ...
            filterShiftsByApplications(q, req.user.id, false, approvedOnly, pendingApprovalsOnly, noApplicationsOnly)
                ...
                //.where('usergroups.user_id', '=', req.user.id)
                .union(function() {
                    // if the user is not a group admin and instead is a privileged user to the group via roles
                    this.select(columns)
                        .from('shifts')
                        .where(function () {
                            _getShiftsYouAreManagingFilterByDates(this, before, after);
